...
const SellerAcquisicionLotes = () => {
  ...
  useEffect(() => {
    ...
    if (filters.searchQuery) {
      ...
      result = result.filter(p => p.sku.toLowerCase().includes(query) || p.nombre.toLowerCase().includes(query));
    }
        ...
        <div className="mb-8">
          ...
          {filteredProducts.length === 0 ? <div className="bg-white rounded-lg p-12 text-center">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                {paginatedProducts.map(product => <ProductCardB2B key={product.id} product={product} onAddToCart={addItem} cartItem={cart.items.find(item => item.productId === product.id)} />)}
              </div>
